...
def density(a,b,charge,orbital_exp,orbital_coe,orbital_pos,X,orbital_typ,position,l,alpha):
    p = 0.5
    n = 30
    D = 0
    L = lebedev()
    for i in range(len(charge)):
        for k in range(1,n+1):
            ...
            for s in L:
                fai = s[1]
                theta = s[0]
                density1 = f(p*(1+x)/(1-x)*math.sin(fai)*math.cos(theta),p*(1+x)/(1-x)*math.sin(fai)*math.sin(theta),p*(1+x)/(1-x)*math.cos(fai),i,charge,position,orbital_coe,orbital_exp,orbital_typ,orbital_pos,X)
                if density1 > 0.0000001:
                    y = (3*density1/4/math.pi)**(-1/6)
                    v = 3.72744
                    c = 12.93532
                    Y1 = y**2+3.72744*y+12.93532
                    Q = (4*c-b**2)**0.5
                    A = 0.0621814
                    y0 = -0.10498
                    Y2 = y0**2+3.72744*y0+12.93532
                    D = D + s[2]*4*math.pi*math.pi/n*2*p**3*(1+x)**2.5/(1-x)**3.5*(-alpha*density1**l+A/2*(math.log(y**2/Y1,math.e)+2*v/Q*math.atan(Q/(2*y+v))-v*y0/Y2*(math.log((y-y0)**2/Y1,math.e)+(2*v+4*y0)/Q*math.atan(Q/(2*y+v)))))*\
...
def xc(charge,position,orbital_coe,orbital_exp,orbital_pos,orbital_typ,X,l,alpha):
    ...
    for i in range(len(orbital_exp)):
        ...
        for j in range(len(orbital_exp)):
            XC.append(density(i,j,charge,orbital_exp,orbital_coe,orbital_pos,X,orbital_typ,position,l,alpha))
        XC_matrix.append(XC)
    return XC_matrix
def program2():
    charge, position, orbital_coe, orbital_exp, orbital_pos, orbital_typ = read_orbital()
    ...
    while TEST == False:
        Energy1 = 0
        H_matrix = np.array(H_matrix)
        XC_matrix = xc(charge, position, orbital_coe, orbital_exp, orbital_pos, orbital_typ,X,1/3,0.9847450219)
